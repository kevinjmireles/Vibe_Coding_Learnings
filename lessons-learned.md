...
24. [Do the Math Before You Build](docs/024-do-the-math-before-you-build.md) — **KISS. Quantify. Clarify.** A rigorous plan for an unvalidated problem is still waste; size probability and impact, prove with arithmetic that the solution's real capacity meets the real need, and make that founding number travel with the work — a correctly sized problem can still be built past within a single day of fast, well-reviewed iteration if nothing forces the number back into view.
